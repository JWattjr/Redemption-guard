...
request_exposure(asset_id, amount_units)          [anyone, fully deterministic]
   latest status == ELIGIBLE and gate_open_at ≤ tx time ?
      record exposure
   otherwise → revert EXPOSURE_BLOCKED
```
...
| Deterministic contract code | Owner-only registration (max 2 assets), URL, amount, and ID validation, the authoritative-domain rule, persistence, the one-hour restricted-to-eligible recovery cooldown, and the exposure gate | Interpreting prose |
...
- **Recovery cooldown:** if the previous latest status is `RESTRICTED` and consensus agrees on a new `ELIGIBLE` result, the assessment is persisted with `gate_open_at = assessment_time + 3600 seconds`. The result is visible immediately, but `request_exposure` remains deterministically blocked until that timestamp. Other `ELIGIBLE` assessments open immediately.
...
| `request_exposure(asset_id, amount_units) → id` | write | integer 1…10^30; latest status must be `ELIGIBLE` and its cooldown (if any) must have elapsed |
